	...
	isLayout(is_layout)
{
   ...
   /*QFile qss(":/res/qss/CustomTitleBar.qss");
	...
	if (closeRelativeWidget != NULL)
	{
		...
		maxNormalSizeButt->setProperty("showMax", is_show_max);
		maxNormalSizeButt->style()->unpolish(maxNormalSizeButt);  //如果以前已经有了样式 在设置属性后需要 手动触发 更新样式
		maxNormalSizeButt->style()->polish(maxNormalSizeButt);
		maxNormalSizeButt->update();
...
void CustomTitleBar::changeMaxNormalSize()
{
    if(closeRelativeWidget!=NULL)
    {
        ...
        is_show_max=!is_show_max;
        //qDebug() << "is_show_max=" <<is_show_max;
        maxNormalSizeButt->setProperty("showMax",is_show_max);   //必须手动刷新样否则不会根据属性变化样式
        maxNormalSizeButt->style()->unpolish(maxNormalSizeButt);
        maxNormalSizeButt->style()->polish(maxNormalSizeButt);
        maxNormalSizeButt->update();
    }
}
...
bool CustomTitleBar::eventFilter(QObject *obj, QEvent *event)
{
	if (event->type() == QEvent::WindowStateChange)
	{
		...
		{
			if (mini_enable && max_normale_enable && closeRelativeWidget != NULL)
			{
				if (is_show_max)
				{
